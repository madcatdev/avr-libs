 ...
 * NOTES
 * Based on https://github.com/asif-mahmud/MIFARE-RFID-with-AVR/tree/master/lib/avr-rfid-library/include
 */ 
...
#include "../../interfaces/spi.h"
	...
	class mfrc522_class{
		public:
		mfrc522_class (spi_class &spi_device, volatile u8 *_portx, volatile u8 *_ddrx, u8 _pinx);
		bool init(void);
		void reset(void);
		void write(u8 reg, u8 data);
		u8 read(u8 reg);
		u8 request(u8 req_mode, u8 * tag_type);
		u8 to_card(u8 cmd, u8 *send_data, u8 send_data_len, u8 *back_data, u32 *back_data_len);
		u8 get_card_serial(u8 *uid);
		private:
		spi_class *spi;
		volatile u8 *portx, *ddrx;
		u8 pinx;
	};
